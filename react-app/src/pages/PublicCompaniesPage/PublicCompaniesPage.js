...
import getPublicCompanies from '../../store/actionCreators/getPublicCompanies';
...
export default function PublicCompaniesPage() {
    ...
    useEffect(() => {
        if (user) dispatch(getPublicCompanies(localStorage.getItem('jwt')))
    }, [dispatch, user])
    ...
    function filter(e) {
        ...
        dispatch(getPublicCompanies(localStorage.getItem('jwt'), {
    ...
    function renderCompanies() {
        return (
            <div className="companies-container">
                {companies.map((company, i) => (<CompanyItem key={i} company={company} publicCompany={true}/>))}
            </div>
        )
    }
